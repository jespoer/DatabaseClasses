...
class Connection { 
	 ...
	 */
	public function get_value($table, $array_select_fields, $array_where_fields, $array_where_values){
	
		/* -- Check if input is valid -- field_array and value_array must be arrays and each value in field_array
		 * must correspond to a value in value_array. Return error_code 0 if not. */
		if(count($array_where_fields) != count($array_where_values) || !is_array($array_where_fields) || !is_array($array_where_values) || !is_array($array_select_fields)){
			return -1;
		} 
	...
	public function get_value_ordered($table, $array_select_fields, $array_where_fields, $array_where_values, $order_by_field, $limit){
		...
		/* user wants to retrieve all rows and columns in array */
		if($select_field_string[0] == '*'){
			$query = "SELECT * FROM ".$table." ORDER BY " .$order_by_field." DESC LIMIT ".$limit;
		...
		/* user wants selected */
			...
			/* make a string that matches the sql syntax for 'where' statements. */
			$where_statements = $this->combine_query_arrays($array_where_fields, $array_where_values,"and");
			$query = "SELECT ".$select_field_string." FROM ".$table." WHERE ".$where_statements." ORDER BY ".$order_by_field." DESC LIMIT ".$limit;
		}
	...
	/* -- COMBINE_QUERY_ARRAYS -- match the field_array and value_array elements to a string with a 'and'
	 * between statements for some special querys to database. returns 0 on failure and string on success */
